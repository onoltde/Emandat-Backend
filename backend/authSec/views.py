...
class userView(APIView):
    ...
    def post(self, request):
        try:
            ...
            user = User(firstname=firstname, lastname=lastname, phonenumber=phonenumber, birthday=birthday, email=email)
...
class competitionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            competition = Competitions(name=name, startdate=startdate, enddate=enddate, description=description, organizer=organizer, uploadeddate=uploadeddate, file=file)
            competition.save()
    ...
    def post(self, request):
        try:
            ...
            if user is not None:
                ...
                tokens_serializer = MyTokenObtainPairSerializer().get_token(user)
                tokens = {
                    'access': str(tokens_serializer.access_token),
                    'refresh': str(tokens_serializer),
                }
                return Response({'message' : 'succeeded', 'data' : serializer.data, 'tokens' : tokens}, status=status.HTTP_200_OK)
            else:
